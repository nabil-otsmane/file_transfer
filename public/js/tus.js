  ...
  _createClass(Upload, [{
      ...
      if (retryDelays != null) {
        if (Object.prototype.toString.call(retryDelays) !== "[object Array]") {
          ...
          this.options.onError = function (err) {
            ...
            if (!shouldRetry(err, _this2._retryAttempt, _this2.options)) {
    ...
    value: function abort(shouldTerminate, cb) {
      ...
      if (shouldTerminate && this.url != null) {
        Upload.terminate(this.url, this.options, function (err) {
          if (err) {
            return cb(err);
          }
    ...
    value: function terminate(url, options, cb) {
      ...
      var retryAttempt = 0;
      var retryIfShould = function retryIfShould(err) {
        if (shouldRetry(err, retryAttempt, options)) {
          var delay = options.retryDelays[retryAttempt++];
          setTimeout(function () {
      ...
      xhr.onload = function () {
        if (xhr.status !== 204) {
          var err = new Error("tus: unexpected response while terminating upload");
          retryIfShould(new _error2.default(err, null, xhr));
          return;
        }
      ...
      xhr.onerror = function (err) {
        retryIfShould(new _error2.default(err, new Error("tus: failed to terminate upload"), xhr));
      };
